...
public abstract class ConnectionWatchdog extends ChannelInboundHandlerAdapter
	...
	public void channelActive(final ChannelHandlerContext ctx) throws Exception {
		NettyGlobal.ctrChannelMap.put(ctx.channel().id().toString(), ctx);
		channel = ctx.channel();
		attempts = 0;
		reconnect = true;
		if (!Check) {
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		NettyGlobal.ctrChannelMap.remove(ctx.channel().id().toString(), ctx);
		disConnect = true;
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		try {
			...
			Global.marketQueue.add(s);
//			logger.info("resv msg:{}",s);
		} catch (Exception e) {
			e.printStackTrace();
